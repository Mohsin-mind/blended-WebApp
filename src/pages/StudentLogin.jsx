...
export default function StudentLogin() {
  ...
  const { trigger: loginTrigger } = useSWRMutation('/login', async (key, { arg }) => {
    return await loginApi(JSON.stringify(arg));
  });

  const { trigger: signupTrigger } = useSWRMutation('/signup', async (key, { arg }) => {
  ...
  const handleFormChange = (formType) => {
    console.log('Form change requested:', formType, 'Current:', activeForm);
  ...
  return (
    <ZodFormProvider key={activeForm} schema={currentSchema} onSubmit={onSubmit}>
      <WebAppLoginLayout 
